 ...
 * @author sseibold
 * 
 *         This class collects all EnvInfoTextBlocks as well as all sub-types like EnvironmentalLabelTextBlock,
 *         FiberTypeTextBlock and DestructionTextBlock.
 */
public class ScaEnvInfoTextBlockProvider implements CatalogContentProvider
{
	@Autowired
	...
	private static final String FSEARCH_QUERY_PRODUCTS = FlexibleSearchQueryUtils.query("SELECT {0} FROM {1} WHERE {2}=?{3}",
			EnvInfoTextBlockModel.PK, EnvInfoTextBlockModel._TYPECODE, EnvInfoTextBlockModel.CATALOGVERSION,
	...
	public Set<ItemModel> getContent(final CatalogVersionModel catalog)
	{
		...
		final Set<ItemModel> textBlocks = new HashSet<ItemModel>();
		textBlocks.addAll(flexibleSearchService.<EnvInfoTextBlockModel> search(query).getResult());

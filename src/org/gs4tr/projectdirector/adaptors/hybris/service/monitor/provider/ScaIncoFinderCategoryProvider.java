 ...
 * 
 */
public class ScaIncoFinderCategoryProvider implements CatalogContentProvider
{
	@Autowired
	...
	private static final String FSEARCH_QUERY_PRODUCTS = FlexibleSearchQueryUtils.query("SELECT {0} FROM {1} WHERE {2}=?{3}",
			IncoFinderCategoryModel.PK, IncoFinderCategoryModel._TYPECODE, IncoFinderCategoryModel.CATALOGVERSION,
	...
	public Set<ItemModel> getContent(final CatalogVersionModel catalog)
	{
		...
		categories.addAll(flexibleSearchService.<IncoFinderCategoryModel> search(query).getResult());

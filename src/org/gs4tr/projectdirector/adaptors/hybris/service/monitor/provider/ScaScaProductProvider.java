 ...
 * 
 */
public class ScaScaProductProvider implements CatalogContentProvider
{
	@Autowired
	...
	private static final String FSEARCH_QUERY_PRODUCTS = FlexibleSearchQueryUtils.query("SELECT {0} FROM {1} WHERE {2}=?{3}",
			ScaProductModel.PK, ScaProductModel._TYPECODE, ScaProductModel.CATALOGVERSION, FSEARCH_PARAM_CATALOG_VERSION);
	...
	public Set<ItemModel> getContent(final CatalogVersionModel catalog)
	{
		...
		products.addAll(flexibleSearchService.<ScaProductModel> search(query).getResult());

 ...
 * 
 */
public class ScaIncoGlobalCategoryProvider implements CatalogContentProvider
{
	@Autowired
	...
	private static final String FSEARCH_QUERY_PRODUCTS = FlexibleSearchQueryUtils.query("SELECT {0} FROM {1} WHERE {2}=?{3}",
			IncoGlobalCategoryModel.PK, IncoGlobalCategoryModel._TYPECODE, IncoGlobalCategoryModel.CATALOGVERSION,
	...
	public Set<ItemModel> getContent(final CatalogVersionModel catalog)
	{
		...
		categories.addAll(flexibleSearchService.<IncoGlobalCategoryModel> search(query).getResult());

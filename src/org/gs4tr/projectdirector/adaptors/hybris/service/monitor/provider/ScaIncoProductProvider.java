 ...
 * 
 */
public class ScaIncoProductProvider implements CatalogContentProvider
{
	@Autowired
	...
	private static final String FSEARCH_QUERY_PRODUCTS = FlexibleSearchQueryUtils.query("SELECT {0} FROM {1} WHERE {2}=?{3}",
			IncoProductModel.PK, IncoProductModel._TYPECODE, IncoProductModel.CATALOGVERSION, FSEARCH_PARAM_CATALOG_VERSION);
	...
	public Set<ItemModel> getContent(final CatalogVersionModel catalog)
	{
		...
		final Set<ItemModel> products = new HashSet<ItemModel>();
		products.addAll(flexibleSearchService.<IncoProductModel> search(query).getResult());

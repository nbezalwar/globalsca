 ...
 * 
 */
public class ScaTargetGroupQuestionnaireTextProvider implements CatalogContentProvider
{
	@Autowired
	...
	private static final String FSEARCH_QUERY_PRODUCTS = FlexibleSearchQueryUtils.query("SELECT {0} FROM {1} WHERE {2}=?{3}",
			TargetGroupQuestionnaireTextModel.PK, TargetGroupQuestionnaireTextModel._TYPECODE,
			TargetGroupQuestionnaireTextModel.CATALOGVERSION, FSEARCH_PARAM_CATALOG_VERSION);
	...
	public Set<ItemModel> getContent(final CatalogVersionModel catalog)
	{
		...
		final Set<ItemModel> texts = new HashSet<ItemModel>();
		texts.addAll(flexibleSearchService.<TargetGroupQuestionnaireTextModel> search(query).getResult());
